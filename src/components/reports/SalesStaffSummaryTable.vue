        ...
        <v-card outlined >
            ...
            <div  v-if="!loading && reports.length > 0">
                <v-app-bar flat>
                    ...
                    <v-spacer></v-spacer>
                    <SalesStaffSummaryPdfDownload :title="title"
                                             :header="header"
                                             :content="reports"
                                             :total="total"
                                             :file-name="fileName"/>
...
<script>
import SalesStaffSummaryPdfDownload from "@/components/reports/shared/SalesStaffSummaryPdfDownload";
import ErrorHandlerMixins from "@/mixins/ErrorHandler";
import ReportAPI from "@/api/ReportAPI";
import EventBus from "@/helpers/EventBus";
export default {
name: "SalesStaffSummaryTable",
    components: {SalesStaffSummaryPdfDownload},
    data () {
        return {
            loading: false,
            reports: [],
    ...
    mounted() {
        EventBus.$on('GENERATE_SALES_STAFF_REPORT', payload => this.generateReport(payload))
    }
